...
def reservations():
    ...
    if request.method == "GET" and "ticket" in request.args and "adult" in request.args and "children" in request.args and "departure" in request.args:
        ...
        tickets = ticket.query.filter(ticket.id==ticketId).join(routes).join(bus).all()
        amount=0
        if(not tickets):
            return redirect("/404")
        else:
            for data in tickets:
                amount = int(data.fee) * int(adult) * int(children)

            reserve=  reservation(userId=session.get("id"),ticketId=ticketId, reservation_number=reservation_number, status=1)
        ...
        return redirect("/")
    return render_template("web/reserve.html", path=path, tickets=tickets)@reserve_route.route("/checkout", methods=["POST", "GET"])
